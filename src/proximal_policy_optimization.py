...
class ProximalPolicyOptimization:
    ...
    def __init__(self, ppo_configuration):
        ...
        self.policy_class = ppo_configuration["policy_class"]
        ...
        if not self.stack_observations:
            ...
            self.sde_actor = self.policy_class(
                self.observation_shape,
                self.action_shape,
                model="sde_actor",
                # action_space_type=self.action_shape_type,
        ...
        #
        # observation = resize(
        #     self.environment.render()[150:350, 200:400, 0] / 255, (80, 80)
        # )
        # for _ in range(100000 * self.number_of_observations_to_stack):
        #     self.batch_observations_test.append(observation)
        #
        # self.batch_observations_test = torch.tensor(
        #     np.asarray(self.batch_observations_test).reshape(
        #         (
        #             -1,
        #             1,
    ...
    def state_dependent_exploration(self, action_probabilities):
        ...
        if (
            ...
            or self.global_timestep_counter > self.total_timesteps * 1
        ):
        ...
        # Exponential Decay:
        self.reweighing_strength = 1 * (
            (0.01 / 1) ** (1 / (self.total_timesteps * 1))
        ...
        if (
            ...
            self.clusterer = hdbscan.HDBSCAN(
                ...
                cluster_selection_method="leaf",  # Try "leaf" for more clusters.
                leaf_size=40,  # Doesn't seem to effect the results much.
            ...
            if self.stack_observations:
                if self.force_image_observation:
                    ...
                    # )
                    observations = np.asarray(self.states_observed).reshape(
                        (
                            -1,
                            1,
                            self.number_of_observations_to_stack,
            ...
            with torch.inference_mode():
                observations = torch.tensor(
                    observations, dtype=torch.float, device="cpu"
                )
                nn_observations = self.sde_actor(observations)
                nn_observations = nn_observations.detach().numpy()
                # print(type(nn_observations), nn_observations.shape)

            # self.clusterer.fit(self.states_observed)

            # TODO: Outlier reduction
            ...
            with torch.inference_mode():
                ...
                nn_observation = nn_observation.detach().numpy()
            # print(
            ...
            # concatanation_start_time = time.perf_counter()
            ...
            #     print(
            #         "Concatanation Time:",
            #         time.perf_counter() - concatanation_start_time,
            #     )
    ...
    def rollout(self):
        ...
        while batch_timesteps_counter < self.timesteps_per_batch:
            ...
            while (
                ...
                # State dependent exploration
                if self.use_state_dependent_exploration:
                    # print("Appended Action:", action, type(int(action)))
