...
class ResultsGenerator:
    ...
    def __init__(self, configuration: dict):
        ...
        self.evaluation_environment = VecFrameStack(
            self.evaluation_environment, n_stack=4
        )
        ...
        for j in range(len(per_training_run_evaluation_statistics[0]["timesteps"])):
            for i in range(self.number_of_training_runs):
                timesteps[f"{(j + 1) * self.evaluation_frequency}"].append(
                    per_training_run_evaluation_statistics[i]["timesteps"][j]
                )
                mins[f"{(j + 1) * self.evaluation_frequency}"].append(
                    per_training_run_evaluation_statistics[i]["results"][j][0]
                )
                maxs[f"{(j + 1) * self.evaluation_frequency}"].append(
                    per_training_run_evaluation_statistics[i]["results"][j][1]
                )
                means[f"{(j + 1) * self.evaluation_frequency}"].append(
                    per_training_run_evaluation_statistics[i]["results"][j][2]
                )
                stds[f"{(j + 1) * self.evaluation_frequency}"].append(
                    per_training_run_evaluation_statistics[i]["results"][j][3]
                )
                medians[f"{(j + 1) * self.evaluation_frequency}"].append(
                    per_training_run_evaluation_statistics[i]["results"][j][4]
                )

        final_evaluation_statistics["Min Timesteps"] = [
            np.NAN for _ in range(len(timesteps))
        ]
        final_evaluation_statistics["Max Timesteps"] = [
            np.NAN for _ in range(len(timesteps))
        ]
        final_evaluation_statistics["Mean Timesteps"] = [
            np.NAN for _ in range(len(timesteps))
        ]
        final_evaluation_statistics["Std Timesteps"] = [
            np.NAN for _ in range(len(timesteps))
        ]
        final_evaluation_statistics["Median Timesteps"] = [
            np.NAN for _ in range(len(timesteps))
        ]

        final_evaluation_statistics["Min Min Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Max Min Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Mean Min Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Std Min Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Median Min Reward"] = [
            np.NAN for _ in range(len(mins))
        ]

        final_evaluation_statistics["Min Max Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Max Max Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Mean Max Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Std Max Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Median Max Reward"] = [
            np.NAN for _ in range(len(mins))
        ]

        final_evaluation_statistics["Min Mean Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Max Mean Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Mean Mean Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Std Mean Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Median Mean Reward"] = [
            np.NAN for _ in range(len(mins))
        ]

        final_evaluation_statistics["Min Std Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Max Std Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Mean Std Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Std Std Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Median Std Reward"] = [
            np.NAN for _ in range(len(mins))
        ]

        final_evaluation_statistics["Min Median Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Max Median Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Mean Median Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Std Median Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
        final_evaluation_statistics["Median Median Reward"] = [
            np.NAN for _ in range(len(mins))
        ]
